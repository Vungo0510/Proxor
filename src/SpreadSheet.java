...
public class SpreadSheet extends JFrame {
    ...
    public String parseSmallerFormula(StringTokenizer tokens, int depth) {
    ...
    public String parseFormula(StringTokenizer tokens, int depth)
        ...
        boolean reachParen = false;
        boolean endParen = false;
        String tokenListBefore = "";
        String tokenListInside = "";
        String tokenListAfter = "";
        String operandBefore = "";
        String operandAfter = "";
        ...
        if (tokens.hasMoreTokens()) {
            while (tokens.hasMoreTokens()) {
                ...
                if (!reachParen) {
                    tokenListBefore += token;
                } else if (reachParen && !endParen) {
                    tokenListInside += token;
                } else if (reachParen && endParen) {
                    tokenListAfter += token;
                }
            }

            if (!tokenListInside.equals("(") && !tokenListAfter.equals(")") && !tokenListBefore.equals("")) {
                operandBefore = tokenListBefore.substring(tokenListBefore.length() - 1, tokenListBefore.length());
                tokenListBefore = tokenListBefore.substring(0, tokenListBefore.length() - 1);

                tokenListInside = tokenListInside.substring(1, tokenListInside.length());
                operandAfter = tokenListAfter.substring(1, 2);
                tokenListAfter = tokenListAfter.substring(2, tokenListAfter.length());
                
                StringTokenizer tokensBeforeParen = new StringTokenizer(tokenListBefore, "+-/*", true);
                StringTokenizer tokensInsideParen = new StringTokenizer(tokenListInside, "+-/*", true);
                StringTokenizer tokensAfterParen = new StringTokenizer(tokenListAfter, "+-/*", true);
                
                String resultBefore = parseSmallerFormula(tokensBeforeParen, depth);
                String resultInside = parseSmallerFormula(tokensInsideParen, depth);
                String resultAfter = parseSmallerFormula(tokensAfterParen, depth);

                String toEvaluate = resultBefore + operandBefore + resultInside + operandAfter + resultAfter;
                StringTokenizer toEvaluateToken = new StringTokenizer(toEvaluate, "+-/*", true);
                String finalResult = parseSmallerFormula(toEvaluateToken, depth);
                return finalResult;
            } else if (tokenListInside.equals("") && tokenListAfter.equals("")){
                StringTokenizer tokensBeforeParen = new StringTokenizer(tokenListBefore, "+-/*", true);
                String finalResult = parseSmallerFormula(tokensBeforeParen, depth);
                return finalResult;
            } else if (tokenListAfter.equals(")") && tokenListBefore.equals("")){
                tokenListInside = tokenListInside.substring(1, tokenListInside.length());
                StringTokenizer tokensInsideParen = new StringTokenizer(tokenListInside, "+-/*", true);
                String finalResult = parseSmallerFormula(tokensInsideParen, depth);
                return finalResult;
            } else if (tokenListAfter.equals(")")){
                operandBefore = tokenListBefore.substring(tokenListBefore.length() - 1, tokenListBefore.length());
                tokenListBefore = tokenListBefore.substring(0, tokenListBefore.length() - 1);
                tokenListInside = tokenListInside.substring(1, tokenListInside.length());
                ...
                String toEvaluate = resultBefore + operandBefore + resultInside;
                StringTokenizer toEvaluateToken = new StringTokenizer(toEvaluate, "+-/*", true);
                String finalResult = parseSmallerFormula(toEvaluateToken, depth);
                return finalResult;
            } else if (tokenListBefore.equals("")) {
                tokenListInside = tokenListInside.substring(1, tokenListInside.length());
                operandAfter = tokenListAfter.substring(1, 2);
                tokenListAfter = tokenListAfter.substring(2, tokenListAfter.length());
                ...
                String toEvaluate = resultInside + operandAfter + resultAfter;
                StringTokenizer toEvaluateToken = new StringTokenizer(toEvaluate, "+-/*", true);
                String finalResult = parseSmallerFormula(toEvaluateToken, depth);
                return finalResult;
            }
